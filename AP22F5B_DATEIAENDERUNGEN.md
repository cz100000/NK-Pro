# AP22F5B – Dateiänderungsliste

## Ergebnis

- **21** bestehende Dateien geändert
- **21** Dateien neu ergänzt
- **0** Dateien gelöscht
- **404** Dateien im finalen Releasebaum

## Geänderte bestehende Dateien

- `CHANGELOG.md`
- `NK-PRO-PROJEKTSTAND.md`
- `README.md`
- `RELEASE_NOTES.md`
- `SHA256SUMS.txt`
- `UI_KOMPONENTENKATALOG.md`
- `UI_MIGRATIONSMATRIX.md`
- `UI_UX_ABNAHMEKATALOG.md`
- `assets/app.css`
- `index.html`
- `js/app-runtime-config.js`
- `js/service-worker-register.js`
- `js/ui-navigation-pages.js`
- `js/ui-page-controller.js`
- `manifest.webmanifest`
- `nk-pro-project.json`
- `package-lock.json`
- `package.json`
- `service-worker.js`
- `ui-reference/index.html`
- `ui-reference/reference.css`

## Neue Dateien

- `AP22F5A_PLANUNGSENTSCHEIDUNGEN.md`
- `AP22F5B_ABNAHMEKATALOG.md`
- `AP22F5B_DATEIAENDERUNGEN.json`
- `AP22F5B_DATEIAENDERUNGEN.md`
- `AP22F5B_DATEIPOSITIVLISTE.md`
- `AP22F5B_PAKETVOLLSTAENDIGKEIT.md`
- `AP22F5B_POSITIVLISTENABGLEICH.md`
- `AP22F5B_PROTECTED_HASHES.json`
- `AP22F5B_README.md`
- `AP22F5B_SCHUTZ_HASH_BERICHT.md`
- `AP22F5B_Screenshots/01_zaehler_desktop.png`
- `AP22F5B_Screenshots/02_zaehler_schmal.png`
- `AP22F5B_Screenshots/03_mietverhaeltnisse_desktop.png`
- `AP22F5B_Screenshots/04_mietverhaeltnisse_schmal.png`
- `AP22F5B_Screenshots/05_mietverhaeltnisse_nur_ansehen.png`
- `AP22F5B_TESTBERICHT.md`
- `AP22F5B_TESTERGEBNISSE.json`
- `AP22F5B_UI_REFERENZ.md`
- `AP22F5B_UMSETZUNGSBERICHT.md`
- `tests/ap22f5b-pages-browser.test.cjs`
- `tests/ap22f5b-pages.test.cjs`

## Gelöschte Dateien

- keine
