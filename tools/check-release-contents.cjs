...
function walk(directory, relative = "") {
  for (const entry of fs.readdirSync(directory, { withFileTypes:true })) {
    ...
    if (/^AP\d+_/i.test(entry.name) && !/^AP15_/i.test(entry.name) && rel !== "AP9_BASELINE_INVENTORY.json") {
      addFinding("historical-ap-artifact", rel, "Historisches AP-Artefakt außerhalb der aktuellen Release-Dokumentation.");
    }
  }
}
