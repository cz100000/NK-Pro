...
function main(){
  ...
  assert(project.appVersion==="99.4.17"&&project.displayVersion==="V99.4.17"&&project.basedOn==="99.4.16","Projektversionsmetadaten sind inkonsistent.");
  assert(project.versionName==="AP13-Briefsteuerung, Schwarzweißdruck und Startnavigation","Versionsname ist inkonsistent.");
  ...
  assert(runtimeConfig.includes('const APP_VERSION = "V99.4.17";')&&runtimeConfig.includes('const APP_VERSION_NAME = "AP13-Briefsteuerung, Schwarzweißdruck und Startnavigation";'),"Laufzeitversion ist inkonsistent.");
  assert(html.includes("<title>NK-Pro V99.4.17 – AP13-Briefsteuerung, Schwarzweißdruck und Startnavigation</title>"),"HTML-Titel ist inkonsistent.");
  assert(worker.includes('const CACHE_NAME = "nk-pro-v99-4-17";'),"Service-Worker-Cache ist inkonsistent.");
  ...
  assert((html.match(/class="nav-icon-svg"/g)||[]).length===23,"Aktuelles Iconinventar einschließlich Start ist inkonsistent.");
  ...
  verifyChecksums();
  process.stdout.write("Release-Konsistenzprüfung abgeschlossen: V99.4.17 mit AP13-Dokumentmodell, app.js 225 Zeilen, 1 Root-State-Ersetzung, 46 seiteneffektfreie Renderer, 75 explizite Wrapper, AP11-Navigation, 13 Controller, 99 Aktionen und vollständige PWA-App-Shell sind konsistent.\n");
}
