...
(async()=>{
  ...
  try{
    ...
    await check(1,"Kompakte Standard-Seitenbreite",async()=>{const size=await page.locator('#manuellewerte .individual-values-page').evaluate(node=>({width:node.getBoundingClientRect().width,max:getComputedStyle(node).maxWidth,left:node.getBoundingClientRect().left,right:innerWidth-node.getBoundingClientRect().right}));assert.ok(size.width<=1180.5,JSON.stringify(size));assert.equal(size.max,"1180px");});
    ...
    const report={generatedAt:new Date().toISOString(),appVersion:'V99.4.59',sourceFile:path.basename(sourcePath),summary:{passed:checks.filter(row=>row.status==='PASS').length,failed:checks.filter(row=>row.status==='FAIL').length,total:checks.length},checks,runtimeErrors:errors};
