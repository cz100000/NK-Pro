...
test("Start, Version, Seitenstruktur und interne Audits sind fehlerfrei", async ({ page }) => {
  ...
  await expect(page).toHaveTitle("NK-Pro V99.4.4 – Migrations-, Sicherungs-, Restore- und Rollback-Fundament");
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      contextHidden: document.querySelector("[data-nav-billing-context-panel]")?.hidden
    };
  });

  expect(result.version).toBe("V99.4.4");
  expect(result.versionName).toBe("Migrations-, Sicherungs-, Restore- und Rollback-Fundament");
...
test("Aktive Abrechnung erscheint nur im geöffneten Arbeitskontext", async ({ page }) => {
  const runtime = attachRuntimeGuards(page);
  await openFreshApp(page);
  const panel = page.locator("[data-nav-billing-context-panel]");
  ...
  await expect(page.locator("[data-nav-billing-context]")).toHaveText("Bearbeitung");
  await expect(page.locator("[data-nav-billing-object]")).not.toHaveText("");
  await expect(page.locator("[data-nav-billing-year]")).not.toHaveText("");
...
test("Archiv ist eigenständig erreichbar und öffnet die Nur-Ansicht", async ({ page }) => {
  ...
  await expect(page.locator("[data-nav-billing-context-panel]")).toBeVisible();
  await expect(page.locator("[data-nav-billing-context]")).toHaveText("Nur Ansicht");
  expect(await page.evaluate(() => isArchiveViewer())).toBe(true);

  await page.evaluate(() => closeArchiveViewer());
  await expect(page.locator("#archiv")).toHaveClass(/active/);
  await expect(page.locator("[data-nav-billing-context-panel]")).toBeHidden();
  runtime.assertClean();
});
...
test("Manifest und PWA-Version stimmen mit der Anwendung überein", async () => {
  ...
  expect(manifest.version).toBe("99.4.4");
  expect(manifest.name).toContain("V99.4.4");
  expect(worker).toContain('const CACHE_NAME = "nk-pro-v99-4-4";');
  expect(worker).toContain('"./index.html"');
  expect(worker).toContain('"./manifest.webmanifest"');
});

test("V99.4.4 lädt produktive Styles und Skripte ausschließlich aus separaten Dateien", async ({ page, request }) => {
  ...
  for (const expected of ["js/navigation.js", "js/modal-events.js", "js/persistence.js", "js/migration.js", "js/backup-recovery.js", "js/archive.js", "js/default-seed.js", "js/app.js", "js/service-worker-register.js"]) {
