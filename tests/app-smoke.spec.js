...
test("Start, Version, Seitenstruktur und interne Audits sind fehlerfrei", async ({ page }) => {
  ...
  await expect(page).toHaveTitle("NK-Pro V99.4.20 – AP17-Bereichs-Dashboards, Navigationslogik und UI-Bereinigung");
  ...
  expect(result.version).toBe("V99.4.20");
  expect(result.versionName).toBe("AP17-Bereichs-Dashboards, Navigationslogik und UI-Bereinigung");
...
test("Manifest und PWA-Version stimmen mit der Anwendung überein", async () => {
  ...
  expect(manifest.version).toBe("99.4.20");
  expect(manifest.name).toContain("V99.4.20");
  expect(worker).toContain('const CACHE_NAME = "nk-pro-v99-4-20-ap17";');
  expect(worker).toContain('"./index.html"');
  expect(worker).toContain('"./manifest.webmanifest"');
});

test("V99.4.20 lädt produktive Styles und Skripte ausschließlich aus separaten Dateien", async ({ page, request }) => {
