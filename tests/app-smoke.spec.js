...
test("Start, Version, Seitenstruktur und interne Audits sind fehlerfrei", async ({ page }) => {
  ...
  await expect(page).toHaveTitle("NK-Pro V99.4.21 – AP18-Korrekturen, UI-Feinschliff und UX-Bereinigung");
  ...
  expect(result.version).toBe("V99.4.21");
  expect(result.versionName).toBe("AP18-Korrekturen, UI-Feinschliff und UX-Bereinigung");
...
test("Manifest und PWA-Version stimmen mit der Anwendung überein", async () => {
  ...
  expect(manifest.name).toContain("V99.4.21");
  expect(worker).toContain('const CACHE_NAME = "nk-pro-v99-4-21-ap18";');
  expect(worker).toContain('"./index.html"');
  expect(worker).toContain('"./manifest.webmanifest"');
});

test("V99.4.21 lädt produktive Styles und Skripte ausschließlich aus separaten Dateien", async ({ page, request }) => {
