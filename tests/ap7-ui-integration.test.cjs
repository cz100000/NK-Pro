...
function main() {
  ...
  assert(definitions.length === 15, `Erwartet wurden 15 Controller, gefunden ${definitions.length}.`);
  assert(actionNames.length === 146 && new Set(actionNames).size === 146, `UI-Aktionsregister ist unvollständig oder doppelt (${actionNames.length}).`);
  ...
  process.stdout.write(`AP7-UI-Architekturprüfung abgeschlossen: 130 Inline-Handler entfernt, 15 Controller, 146 eindeutige Aktionen, zentraler 5-Ereignis-Vertrag und keine DOM-Ereignisregistrierung in app.js.\n`);
}
