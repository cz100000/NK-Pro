...
function main() {
  ...
  assert(actionNames.length === 99 && new Set(actionNames).size === 99, `UI-Aktionsregister ist unvollständig oder doppelt (${actionNames.length}).`);
  ...
  process.stdout.write(`AP7-UI-Architekturprüfung abgeschlossen: 130 Inline-Handler entfernt, 13 Controller, 99 eindeutige Aktionen, zentraler 5-Ereignis-Vertrag und keine DOM-Ereignisregistrierung in app.js.\n`);
}
