...
test("Persistenz-, Migrations- und Archivmodule sind vor app.js geladen", async ({ page }) => {
  const runtime = attachRuntimeGuards(page);
  await openFreshApp(page);

  const result = await page.evaluate(() => {
    const scripts = [...document.scripts].map(script => new URL(script.src).pathname.split("/").pop());
    const names = ["persistence.js", "migration.js", "archive.js", "default-seed.js", "app.js"];
  ...
  expect(result.modules).toEqual({ persistence:true, migration:true, archive:true });
  expect(result.frozen).toEqual({ persistence:true, migration:true, archive:true });
