(function (global) {
  ...
  const REGISTRY = Object.freeze([
    ...
    rule("NKP-PLAU-012","Gesamtwasserverbrauch und individuelle Messperioden stimmen überein",CATEGORY.PLAUSIBILITY,"consumption",{dataSource:"kostenarten[K002].gesamtverbrauch und zaehlerDaten.messperioden",targetTab:"manuellewerte",targetSelector:"#individualWaterComparison",solution:"Wasserzählerstände, Zuordnungen und den Gesamtverbrauch unter Gesamtkosten prüfen.",allowNotApplicable:true}),
    ...
    rule("NKP-FACH-020","NK- und Kaltmietkorrekturen sind getrennt und berechenbar",CATEGORY.MANDATORY,"prepayments",{dataSource:"mieter.vorjahresKorrektur und mieter.kaltmietKorrektur",targetTab:"einnahmen"}),
  ...
  function evaluateConsumption(data, ctx, prior) {
    ...
    let individualWater=null; try{individualWater=global.NKProBillingCalculation&&global.NKProBillingCalculation.waterConsumptionSummary?global.NKProBillingCalculation.waterConsumptionSummary(data):null;}catch(_){}
    if(!individualWater||numSafe(individualWater.totalMeters)<=0)results.push(notApplicable("NKP-PLAU-012","Keine individuellen Wasserzähler für den Gesamtverbrauchsabgleich vorhanden.",data));
    else if(numSafe(individualWater.expected)<=0)results.push(notApplicable("NKP-PLAU-012","Unter Gesamtkosten ist kein Gesamtwasserverbrauch als Vergleichswert vorhanden.",data));
    else if(Math.abs(numSafe(individualWater.difference))>.01)results.push(result("NKP-PLAU-012",{details:"Gesamtkosten "+formatPlainNumber(individualWater.expected,2)+" m³, individuelle Messperioden "+formatPlainNumber(individualWater.actual,2)+" m³ (Differenz "+formatPlainNumber(individualWater.difference,2)+" m³).",values:{expected:individualWater.expected,actual:individualWater.actual,difference:individualWater.difference,tolerance:.01}},data));
    else results.push(passed("NKP-PLAU-012","Gesamtwasserverbrauch und individuelle Messperioden stimmen innerhalb von 0,01 m³ überein.",{expected:individualWater.expected,actual:individualWater.actual,difference:individualWater.difference,tolerance:.01},data));
    return results;
  }
  ...
  function evaluateFinancial(data, ctx) {
    ...
    const invalidCorrections=ctx.billable.filter(t=>{const nk=Number(String(t.vorjahresKorrektur??0).replace(",",".")),rent=Number(String(t.kaltmietKorrektur??0).replace(",","."));return !Number.isFinite(nk)||!Number.isFinite(rent);});
    if(invalidCorrections.length)invalidCorrections.forEach(t=>results.push(result("NKP-FACH-020",{entity:tenantEntity(t),details:"Mindestens ein Korrekturwert ist nicht numerisch berechenbar.",values:{nk:t.vorjahresKorrektur,rent:t.kaltmietKorrektur}},data)));else results.push(passed("NKP-FACH-020","NK- und Kaltmietkorrekturen sind getrennt vorhanden und berechenbar.",{count:ctx.billable.length},data));
    ...
    if(totals){const delta=numSafe(totals.allocationDelta); if(Math.abs(delta)>.02)results.push(result("NKP-FACH-016",{details:"Aktive Kosten und Verteilung weichen um "+fmtMoney(delta)+" ab.",values:{allocationDelta:delta,totalCosts:totals.totalCosts,allTenantShare:totals.allTenantShare}},data));else results.push(passed("NKP-FACH-016","Aktive Kosten und Verteilung stimmen überein.",{allocationDelta:delta},data));
      const owner=numSafe(totals.ownerShare),documented=numSafe(totals.documentedOwnerShare),unexplained=owner-documented; if(Math.abs(unexplained)>.02)results.push(result("NKP-PLAU-010",{details:"Nicht erklärter Restanteil: "+fmtMoney(unexplained)+" (Eigentümer-/Restanteil "+fmtMoney(owner)+", dokumentierter Leerstand "+fmtMoney(documented)+").",values:{ownerShare:owner,documentedOwnerShare:documented,unexplainedOwnerShare:unexplained,totalCosts:totals.totalCosts}},data));else results.push(passed("NKP-PLAU-010","Kein unerklärter Restanteil vorhanden.",{ownerShare:owner,documentedOwnerShare:documented,unexplainedOwnerShare:unexplained},data));
