...
(function (global) {
  ...
  function manualBody(cost, a, external) {
    ...
    return sourceSelector + controls + valuesTable(cost,!readonly) + mismatch + '<div class="individual-cost-actions"><button type="button" class="ui-button ui-button--primary" data-ui-action="application.save" ' + (readonly ? 'disabled' : '') + '>Speichern</button><button type="button" class="ui-button ui-button--danger" data-individual-reset="' + html(cost.id) + '" ' + (readonly ? 'disabled' : '') + '>Werte zurücksetzen</button></div>';
  }
