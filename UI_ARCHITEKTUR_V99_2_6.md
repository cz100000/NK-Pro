...
- statische HTML- und ID-Strukturprüfung aller betroffenen Tabs,
- JavaScript-Syntaxprüfung aller Scriptblöcke mit Node.js,
- gezielte Ausführung der aus `index.html` extrahierten Originalfunktionen für Status-, Hauszähler- und Warnlogik,
- Quellcodeprüfungen der zentralen Renderer, Klappboxreihenfolgen, Summenzeilen und CSS-Masterregeln,
- ZIP-Integritätsprüfung und SHA-256-Prüfsummen.

Eine echte lokale Chromium-Navigation wurde durch eine verwaltete Browserrichtlinie der Ausführungsumgebung blockiert. Daher wird kein bestandener Live-Browsertest behauptet. GitHub Pages, Service Worker und PWA-Installation müssen nach Veröffentlichung mit einem kurzen Smoke-Test geprüft werden.
...
## Ergänzung V99.2.5 – verbindliches Navigations-Mock-up

V99.2.5 ersetzt die kartenartige Baumdarstellung der V99.2.3 vollständig durch die freigegebene flache Sidebar-Gestaltung. Die Navigation besitzt keine aufklappbaren Hauptkarten mehr. Übersicht, Stammdaten, Abrechnung und System sind statische visuelle Bereiche; nur die vier Abrechnungsphasen sind aufklappbar.

### Zentrale Komponenten

- `sidebar-brand`: Logo, NK-PRO und oberer Einklapppfeil.
- `sidebar-billing-bar`: dynamisches Abrechnungsjahr und Bearbeitungsstatus.
- `nav-static-section`: Übersicht, Stammdaten und System.
- `nav-phase` / `nav-phase-toggle` / `nav-phase-children`: Accordion der vier Workflowphasen.
- `nav-status-dot`: blauer Punkt für den aktiven und grauer Punkt für inaktive Unterpunkte.
- `sidebar-bottom-collapse`: Abschlussaktion „Navigation einklappen“.

### Interaktionsregeln

`setOpenPhase()` hält exakt eine Phase geöffnet. `ensureNavigationPath()` öffnet bei jedem direkten oder programmgesteuerten Tabwechsel die zum Zieltab gehörende Phase. Der Phasenzustand wird unter `nkpro.workflowNavigation.v2` gespeichert. Die Desktop-Seitenleiste wird über `sidebar-collapsed` vollständig ausgeblendet und über den vorhandenen Schalter in der globalen Kopfzeile wieder geöffnet; mobil bleibt das Off-Canvas-Verhalten erhalten.

### Geschützte Fachlogik

Die Änderung betrifft HTML-Struktur, CSS und Navigationssteuerung. Technische Tab-IDs, Fachrenderer, Berechnungsfunktionen und Datenmodell bleiben unverändert.


## Ergänzung V99.2.5 – Breitenanpassung

Die Seitenleiste wurde ausschließlich in der Breite von 360 px auf 324 px reduziert. Dies entspricht exakt 10 %. Die mobile Maximalbreite wurde identisch angepasst. Alle übrigen visuellen und interaktiven Vorgaben des freigegebenen Mock-ups bleiben unverändert.


## Ergänzung V99.2.6 – Status-, Zähler- und Umlagestruktur

### Zentraler Seitenstatus

`updateAllPageHeaders()` bildet Bearbeitung, Finalisierung, Archivansicht und fehlende aktive Abrechnung auf eine gemeinsame Statusplakette ab. Die Statusfarbe wird über die Klassen `is-working`, `is-finalized`, `is-archived` und `is-none` gesteuert. Die spezielle Regel `.readonly-badge[hidden]` verhindert, dass ausgeblendete Alt-Plaketten durch die allgemeine Badge-Darstellung sichtbar werden.

### Einzelstatus in der Prüfungskachel

`renderOverviewCards()` rendert jeden Prüfpunkt mit eigenem Status und Symbol. Der Gesamtstatus der Karte bleibt erhalten; er ersetzt nicht den Status einzelner Zeilen.

### Zählerarchitektur

Der Tab `wasser` besitzt fünf statische Fachabschnitte. `meterHouseSection` steht vor der Wohnungszählererfassung und speichert Hauszähler- und Rechnungswerte getrennt in `state.waterMeters.settings`. `houseMeterConsumption()` berechnet ausschließlich die Differenz von Hauszähler-End- und Anfangsstand. `waterDifferenceState()` liefert die gemeinsame Plausibilitätsdarstellung.

Die Tabelle K002 erzeugt eine `tfoot`-Summenzeile aus den berechneten Verbrauchsspalten. Filterleisten erhalten innerhalb von `.meter-section-body` dieselbe Breite und denselben horizontalen Außenabstand wie `.table-wrap`.

### Umlage und Vorauszahlungsregeln

Die Umlageansicht beginnt direkt mit den Ergebnissen. `calculateUmlage()` bleibt die zentrale Live-Berechnung; der frühere UI-Aufruf `recalculateAll()` ist aus der Oberfläche entfernt. Die Reset-Aktion verbleibt mit Bestätigungsdialog im Eingabeabschnitt.

Die Einstellungen der Vorauszahlungsanpassung werden als `.prepayment-settings-grid` mit zwei Spalten ausgegeben und unterhalb von 760 px auf eine Spalte reduziert.
